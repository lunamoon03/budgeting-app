...
pub(super) fn write_to_file(file_path: &str, accounts: Vec<Account>) -> Result<(), Box<dyn Error>> {
...
pub(super) fn read_from_file(contents: String) -> Result<Vec<Account>, Box<dyn Error>> {
...
fn read_account(split: &Vec<&str>, mut iter: usize) -> Result<(Account, usize), Box<dyn Error>> {
...
mod tests {
    ...
    fn empty_file() {
        ...
        assert!(read_from_file(empty).unwrap().is_empty());
    }

    #[test]
    #[should_panic]
    fn bad_start() {
        let _ = read_from_file(get_file_contents("src/test-files/bad-start.csv").unwrap()).unwrap();
    }

    #[test]
    #[should_panic]
    fn bad_end() {
        let _ = read_from_file(get_file_contents("src/test-files/bad-end.csv").unwrap()).unwrap();
    }

    #[test]
    #[should_panic]
    fn bad_trans_label() {
        let _ = read_from_file(get_file_contents("src/test-files/bad-trans-label.csv").unwrap()).unwrap();
    }

    #[test]
    fn one_account() {
        let a = get_file_contents("src/test-files/1-account.csv").unwrap();
        assert_eq!(&format!("{}", read_from_file(a).unwrap().get(0).unwrap()),
    ...
    #[test]
    fn two_accounts() {
        let b = read_from_file(get_file_contents("src/test-files/2-account.csv").unwrap()).unwrap();
    ...
    fn write_read_test() {
        ...
        let binding = read_from_file(file_contents).unwrap();
